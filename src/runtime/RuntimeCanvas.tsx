...
export default function RuntimeCanvas({ project, onReady, onError }: RuntimeCanvasProps) {
  ...
  useEffect(() => {
    ...
    return () => { disposed = true; window.removeEventListener('tomcat:engine:log', log); window.removeEventListener('tomcat:engine:error', log); try { runtime?.shutdown?.(); } catch { /* module may not expose shutdown */ } };
  }, [project, onError, onReady]);
  return <div className="runtime-host" style={{ width: '100%', height: '100%', margin: 0, overflow: 'hidden', background: '#202020', color: '#d9d9d9' }}><canvas ref={canvasRef} id="canvas" tabIndex={0} aria-label="TomCat Engine WebGL2 editor" style={{ width: '100%', height: '100%', display: 'block', outline: 0 }} />{error ? <div className="runtime-loading runtime-error"><strong>启动失败</strong><span>{error}</span></div> : !ready && <div className="runtime-loading"><strong>TomCat Engine</strong><span>{status}</span></div>}</div>;
}
